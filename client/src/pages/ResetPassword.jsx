...
const ResetPassword = () => {
  ...
  return (
    <div className='flex justify-center items-center min-h-screen sm:px-24'>
      <img onClick={() => navigate('/')} src={assets.logo} alt='Logo' className='absolute left-5 sm:left-20 top-5 w-28 sm:w-32 cursor-pointer mb-8' />

      <form className='bg-slate-900 rounded-lg p-8 shadow-lg w-96 text-sm'>

        <h1 className='text-white text-2xl text-center mb-4 font-semibold'>Reset Password</h1>
        <p className='text-indigo-300 mb-6 text-center'>Enter your E-Mail address.</p>
        ...
        <div className='mb-4 flex items-center w-full px-5 py-2.5 rounded-full gap-3 bg-[#333A5C]'>
          ...
          <input type="email" placeholder='E-Mail Id' className='bg-transparent outline-none text-white' value={email} onChange={e => setEmail(e.target.value)} required />

        <button className='w-full py-2 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-900 text-center text-white'>Submit</button>
        </div>
      </form>
...
{/* otp to reset password */}
      <form className='bg-slate-900 rounded-lg p-8 shadow-lg w-96 text-sm'>
        <h1 className='text-white text-2xl text-center mb-4 font-semibold'>Reset Password OTP</h1>
